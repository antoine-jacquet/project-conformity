...
png("~/Documents/GitHub/project-conformity/conformity_graphs/sigmoid/sigmoid_step_color.png")	
	...
	plot(id, 0, 1, lwd=3,						# plot of the identity function (random copying)
	...
	for (k in 1:5) {
		...
		curve(x + k/5*x*(1-x)*(2*x-1), x1, x2, col="red", lwd=width, add=TRUE);
		curve(x + k/5*x*(1-x)*(2*x-1), 0, x1, col="red", lwd=width, lty=1, add=TRUE);
		curve(x + k/5*x*(1-x)*(2*x-1), x2, 1, col="red", lwd=width, lty=1, add=TRUE);
		curve(x + k/5*x*(1-x)*(2*x-1), 0, x1, col="yellow", lwd=width, lty=2, add=TRUE);
		curve(x + k/5*x*(1-x)*(2*x-1), x2, 1, col="yellow", lwd=width, lty=2, add=TRUE);
	}
	
	# Quasi-step function
	curve(1/2 + s*(x-1/2), from = xlow, to= xhigh, col="blue", lwd=3, add=TRUE) ;
	curve(s0*x/x, from=0, to=xlow, col="blue", lwd=3, add=TRUE) ;
	curve((1-s0)*x/x, from=xhigh, to=1, col="blue", lwd=3, add=TRUE) ;
